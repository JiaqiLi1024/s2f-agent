...
RESULT_JSON_NAME = "protein_structure_visualize.result.json"
...
HEADERS = {"User-Agent": "s2f-agent-protein-structure-visualize/0.1"}

SS_COLORS = {"H": "#D62728", "E": "#1F77B4", "C": "#AAAAAA", "-": "#AAAAAA"}
SS_LABELS = {"H": "Helix", "E": "Strand", "C": "Coil / Loop", "-": "Coil / Loop"}
SECONDARY_HIGHLIGHT_COLORS = {"H": "#D62728", "E": "#1F77B4", "C": "#2CA02C"}
SECONDARY_ALIASES = {
    "h": "H",
    "helix": "H",
    "alpha": "H",
...
def collect_generated_outputs(outdir: str, prefix: str) -> Dict[str, List[str]]:
    ...
    for name in sorted(os.listdir(outdir)):
        ...
        if not os.path.isfile(path):
            continue
        if name == RESULT_JSON_NAME:
            continue

        if name in {"structure_summary.tsv", "summary.txt"}:
            bucket = "summary"
        elif name.endswith((".pdb", ".cif")):
            bucket = "structure"
        elif name.endswith("_view.html"):
            bucket = "viewer"
        elif "contact_map" in name or "ca_distances" in name:
            bucket = "contact_map"
        elif "bfactor" in name:
            bucket = "bfactor"
        elif "secondary_structure" in name:
            bucket = "secondary"
        elif "pocket" in name or "sasa_per_residue" in name:
            bucket = "pocket"
        elif "conservation" in name:
            bucket = "conservation"
        elif "ppi_" in name or "ppi." in name:
            bucket = "ppi"
...
def aa1_from_resname(resname: str) -> str:
    """Convert a PDB 3-letter residue name to a 1-letter amino-acid code."""
    return AA3_TO_1.get(str(resname).strip().upper(), "X")


def parse_highlight_residue_specs(raw: Optional[str], default_chain: str) -> List[Dict[str, Any]]:
    """Parse comma-separated residue specs such as 3-D, A:3-D, A:3:ASP, A:3."""
    if not raw:
        return []
    ...
    for token in raw.replace("+", ",").replace(";", ",").split(","):
        ...
        parts = item.split(":")
        if len(parts) == 3:
            chain, residue_part, expected = parts[0].strip(), parts[1].strip(), parts[2].strip()
        elif len(parts) == 2:
            first, second = parts[0].strip(), parts[1].strip()
            if first and not first.lstrip("-").isdigit():
                chain, residue_part = first, second
            else:
                residue_part, expected = first, second

        if "-" in residue_part:
            residue_part, suffix = residue_part.split("-", 1)
            expected = expected or suffix

        try:
            resseq = int(residue_part.strip())
        except ValueError as exc:
            raise ValueError(f"Invalid residue highlight spec '{item}'. Use formats like 3-D, A:3-D, A:3:ASP, or A:3") from exc

        expected = expected.strip().upper()
        if len(expected) == 3:
            expected = AA3_TO_1.get(expected, expected)
        if expected and len(expected) != 1:
            raise ValueError(f"Invalid amino-acid code in highlight spec '{item}'. Use one-letter or three-letter residue code")

        specs.append({"raw": item, "chain": chain, "resseq": resseq, "expected_aa": expected})
    return specs


def resolve_highlight_residues(
    res_df: pd.DataFrame,
    specs: List[Dict[str, Any]],
    warnings: List[str],
    ...
    indexed = res_df.copy()
    indexed["aa1"] = indexed["resname"].map(aa1_from_resname)
    for spec in specs:
        hit = indexed[(indexed["chain"] == spec["chain"]) & (indexed["resseq"] == spec["resseq"])]
        if hit.empty:
            missing.append(spec)
            warnings.append(f"highlight_residue_not_found:{spec['raw']}")
            continue
        row = hit.iloc[0]
        aa1 = str(row["aa1"])
        if spec.get("expected_aa") and spec["expected_aa"] != aa1:
            warnings.append(
                f"highlight_residue_aa_mismatch:{spec['raw']}:actual-{aa1}"
            )
        label = f"{row['chain']}:{int(row['resseq'])}-{aa1}"
        resolved.append({
            "raw": spec["raw"],
            "chain": row["chain"],
...
def parse_highlight_secondary(raw: Optional[str]) -> List[str]:
    if not raw:
        return []
    selected: List[str] = []
    for token in raw.replace("+", ",").replace(";", ",").split(","):
        ...
        if not key:
            continue
        if key not in SECONDARY_ALIASES:
            raise ValueError(
                f"Unknown secondary-structure highlight '{token}'. Use H/E/C or helix/sheet/coil"
            )
        ss = SECONDARY_ALIASES[key]
        if ss not in selected:
            selected.append(ss)
    return selected


def pdb_has_secondary_records(pdb_path: str) -> bool:
    """Return True when a PDB file contains author-provided HELIX/SHEET records."""
    try:
...
def secondary_assignment_warning(pdb_path: str) -> Optional[str]:
    if shutil.which("mkdssp"):
        return None
    if pdb_has_secondary_records(pdb_path):
        return None
    return (
        "mkdssp was not found and the PDB file has no HELIX/SHEET records; "
        "secondary-structure modules cannot assign helices or sheets reliably. "
        "Install DSSP/mkdssp or use a PDB file with HELIX/SHEET annotations."
    )


def compute_secondary_structure_df(
        ...
        for chain in model.get_chains():
            ...
            for res in chain.get_residues():
                ...
                for ch, start, end in helix_ranges:
                    if ch == cid and start <= resseq <= end:
                        ss = "H"
                        break
                if ss == "-":
    ...
    if not ss_data:
        return pd.DataFrame(columns=["chain", "resseq", "ss", "ss_simple"])

    ss_df = pd.DataFrame(ss_data).sort_values(["chain", "resseq"]).reset_index(drop=True)
    ss_df["ss_simple"] = ss_df["ss"].map(lambda s: "H" if s == "H" else ("E" if s in "BE" else "C"))
    return ss_df


def render_highlight_viewer(
    pdb_path: str,
    prefix: str,
    outdir: str,
    ...
    """Render an HTML viewer with residue and/or secondary-structure highlights."""
    try:
        import py3Dmol  # type: ignore
    except ImportError:
        raise ImportError("py3Dmol is required for highlight HTML. Install with: pip install py3Dmol")

    with open(pdb_path, "r") as fh:
        pdb_data = fh.read()

    view = py3Dmol.view(width=950, height=680)
    view.addModel(pdb_data, "pdb")
    view.setStyle({}, {"cartoon": {"color": "lightgray", "opacity": 0.45}})

    legend_items: List[str] = []
    highlighted_secondary_count = 0
    if selected_secondary and not ss_df.empty:
        for ss in selected_secondary:
            color = SECONDARY_HIGHLIGHT_COLORS.get(ss, "#2CA02C")
            ss_hits = ss_df[ss_df["ss_simple"] == ss]
            highlighted_secondary_count += len(ss_hits)
            for chain_id, chain_grp in ss_hits.groupby("chain"):
                resi_str = _resi_string(chain_grp["resseq"].astype(int).tolist())
                sel = {"chain": chain_id, "resi": resi_str}
                view.addStyle(sel, {"cartoon": {"color": color, "opacity": 1.0}})
            legend_items.append(
                ...
                f'display:inline-block;margin-right:5px;border:1px solid #777;"></span>'
                f'{SS_LABELS.get(ss, ss)} ({len(ss_hits)} residues)</span>'
            )

    for hit in residue_hits:
        sel = {"chain": hit["chain"], "resi": str(hit["resseq"])}
        ca_sel = {"chain": hit["chain"], "resi": str(hit["resseq"]), "atom": "CA"}
        view.addStyle(sel, {"stick": {"color": highlight_color, "radius": 0.22}})
        view.addStyle(ca_sel, {"sphere": {"color": highlight_color, "radius": 0.75}})
        if label_residues:
            view.addLabel(
                hit["label"],
                {
    ...
    if residue_hits:
        legend_items.append(
            f'<span style="display:inline-flex;align-items:center;margin:0 10px 6px 0;">'
            f'<span style="width:14px;height:14px;border-radius:50%;background:{highlight_color};'
            f'display:inline-block;margin-right:5px;border:1px solid #777;"></span>'
            f'Highlighted residues ({len(residue_hits)})</span>'
        )

    if residue_hits:
        first = residue_hits[0]
        view.zoomTo({"chain": first["chain"], "resi": str(first["resseq"])})
    ...
    residue_rows = "".join(
        f"<tr><td>{h['chain']}</td><td>{h['resseq']}</td><td>{h['resname']}</td><td>{h['aa1']}</td><td>{h['label']}</td></tr>"
        for h in residue_hits
    )
    residue_table = (
        "<table><thead><tr><th>Chain</th><th>Residue</th><th>3-letter</th><th>1-letter</th><th>Label</th></tr></thead>"
        f"<tbody>{residue_rows}</tbody></table>"
        if residue_hits else "<p>No residue-specific highlights requested.</p>"
    )
    legend_html = "".join(legend_items) or "No highlight selections rendered."
    viewer_html = view._make_html()
    html_content = f"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Protein Highlight Viewer - {prefix}</title>
  <style>
    body {{ font-family: Arial, sans-serif; background: #f6f7f9; margin: 0; padding: 20px; color: #222; }}
    h2 {{ margin: 0 0 6px; }}
    .subtitle {{ color: #666; font-size: 13px; margin-bottom: 14px; }}
    .viewer-wrap {{ background: white; border: 1px solid #ddd; border-radius: 6px; display: inline-block; padding: 12px; }}
    .legend {{ margin: 12px 0; font-size: 13px; }}
    table {{ border-collapse: collapse; margin-top: 12px; background: white; font-size: 13px; }}
    th, td {{ border: 1px solid #ddd; padding: 6px 9px; text-align: left; }}
    th {{ background: #f0f2f5; }}
  </style>
</head>
<body>
  <h2>Protein Highlight Viewer - {prefix}</h2>
  <div class="subtitle">Residue labels use chain:position-one-letter amino-acid notation.</div>
  <div class="viewer-wrap">{viewer_html}</div>
  <div class="legend">{legend_html}</div>
  {residue_table}
</body>
</html>"""

    out_path = os.path.join(outdir, f"{prefix}.highlight.html")
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write(html_content)

    summary = {
        "highlight_html": out_path,
        "n_highlighted_residues": len(residue_hits),
        "selected_secondary": selected_secondary,
        "n_secondary_residues": int(highlighted_secondary_count),
    }
    print(f"[INFO] Highlight viewer saved: {out_path}")
...
def _submit_jackhmmer(sequence: str, n_iterations: int = 1, evalue: float = 1e-4) -> str:
    """
    Submit a jackhmmer search to the current EBI HMMER API and return the job UUID.

    The legacy `/Tools/hmmer/search/jackhmmer` route is now a web UI route and
    rejects POST with HTTP 405. The v1 API accepts JSON at `/api/v1/search/jackhmmer`.
    """
    payload = {
        "database":   "refprot",
        "input":      f">query\n{sequence}",
    ...
    url = f"{EBI_HMMER_API}/search/jackhmmer"
    r = requests.post(url, json=payload, headers=headers, timeout=60)
    if r.status_code >= 400:
        raise RuntimeError(f"EBI HMMER submission failed (status {r.status_code}): {r.text[:500]}")
    data = r.json()
    job_id = data.get("id")
    if not job_id:
        raise RuntimeError(f"EBI HMMER submission did not return a job id: {data}")
    print(f"[INFO] EBI HMMER job submitted: {job_id}")
    return str(job_id)


def _decode_download_response(response: requests.Response) -> str:
...
def _download_jackhmmer_stockholm(job_id: str, poll_interval: int = 5, max_wait: int = 120) -> str:
    """Request and download the Stockholm MSA file for a completed HMMER job."""
    import time

    formats = ("stockholm", "sto")
    headers = {**HEADERS, "Accept": "application/json"}

    for fmt in formats:
        generate_url = f"{EBI_HMMER_API}/download/{job_id}/{fmt}"
        r = requests.post(generate_url, headers=headers, timeout=30)
        if r.status_code not in (200, 202, 204, 409, 422):
            r.raise_for_status()
            ...
            if downloads.status_code == 200:
                for item in downloads.json():
                    ...
                    if "stockholm" in haystack and url and status not in {"pending", "running"}:
                        ...
                        text = _decode_download_response(file_response)
                        if "# STOCKHOLM" in text:
                            return text

            direct = requests.get(generate_url, headers=HEADERS, timeout=60)
            if direct.status_code == 200:
                text = _decode_download_response(direct)
                if "# STOCKHOLM" in text:
                    return text
...
def _poll_jackhmmer(uuid: str, poll_interval: int = 5, max_wait: int = 300) -> str:
    ...
    waited = 0
    while waited < max_wait:
        r = requests.get(f"{EBI_HMMER_API}/search/{current_id}", headers=HEADERS, timeout=30)
        r.raise_for_status()
        data = r.json()
        ...
        if status in {"FAILURE", "REVOKED"}:
            raise RuntimeError(f"EBI HMMER job failed with status {status}.")
    ...
    return _download_jackhmmer_stockholm(current_id)
...
def main():
    ...
    parser.add_argument(
        "--modules", default="all",
        help=(
            "Comma-separated list: view, contact_map, bfactor, secondary, pocket, "
            "conservation, ppi, highlight. "
    ...
    # Highlight HTML viewer
    parser.add_argument(
        "--highlight-residues",
        default=None,
        help=(
            "Comma-separated residue highlights. Formats: 3-D, A:3-D, A:3:ASP, A:3. "
            "When chain is omitted, --chain or the selected chain is used."
        ),
    )
    parser.add_argument(
        "--highlight-secondary",
        default=None,
        help="Comma-separated secondary structure classes to highlight: H/E/C or helix/sheet/coil.",
    )
    parser.add_argument("--highlight-color", default="#FFB000",
        help="Hex colour for residue-specific highlights.")
    parser.add_argument("--no-highlight-labels", action="store_true",
        help="Disable residue labels in the highlight HTML viewer.")
    ...
    # Parse modules
    all_modules = {"view", "contact_map", "bfactor", "secondary", "pocket", "ppi", "conservation", "highlight"}
    default_modules = all_modules - {"highlight"}
    ...
    if args.highlight_residues or args.highlight_secondary:
        modules.add("highlight")
    ...
    if args.chain and args.chain not in available_chains:
        message = f"Chain '{args.chain}' not found. Using chain '{chain_id}'."
    ...
    # Run modules
    # ----------------------------------------------------------
    if "secondary" in modules or args.highlight_secondary:
        message = secondary_assignment_warning(pdb_path)
        if message:
            warnings.append(message)
            print(f"[WARN] {message}")
    ...
    if "view" in modules:
        ...
        try:
            ...
            message = f"View module skipped: {ex}"
            warnings.append(message)
            print(f"[WARN] {message}")
    ...
    if "highlight" in modules:
        print(f"[INFO] Rendering highlight HTML viewer")
        try:
            residue_specs = parse_highlight_residue_specs(args.highlight_residues, default_chain=chain_id)
            residue_hits, residue_missing = resolve_highlight_residues(res_df, residue_specs, warnings)
            selected_secondary = parse_highlight_secondary(args.highlight_secondary)
            if not residue_hits and not selected_secondary:
                message = "Highlight module requested but no valid residues or secondary-structure classes were selected."
                warnings.append(message)
                print(f"[WARN] {message}")
            else:
                ss_df = (
                ...
                if selected_secondary and ss_df.empty:
                    message = "Secondary-structure highlight requested but no secondary structure assignments were available."
                    warnings.append(message)
                    print(f"[WARN] {message}")
                highlight_html, highlight_summary = render_highlight_viewer(
                    pdb_path,
                    prefix,
                    args.outdir,
                    residue_hits,
                    ...
                    label_residues=not args.no_highlight_labels,
                )
                summary["highlight_html"] = highlight_html
                summary["n_highlighted_residues"] = highlight_summary["n_highlighted_residues"]
                summary["n_missing_highlight_residues"] = len(residue_missing)
                summary["highlight_secondary"] = ",".join(highlight_summary["selected_secondary"])
                summary["n_highlighted_secondary_residues"] = highlight_summary["n_secondary_residues"]
                if selected_secondary and highlight_summary["n_secondary_residues"] == 0:
                    message = (
                        "Secondary-structure highlight selected but no residues matched. "
                        "Install DSSP/mkdssp or use a PDB file with HELIX/SHEET records."
                    )
                    warnings.append(message)
                    print(f"[WARN] {message}")
        except ImportError as ex:
            message = f"Highlight module skipped: {ex}"
            warnings.append(message)
            print(f"[WARN] {message}")
    ...
    if "conservation" in modules:
        ...
        try:
            ...
            message = f"Conservation module failed: {ex}"
            warnings.append(message)
            print(f"[WARN] {message}")
    ...
    if "ppi" in modules:
        gene = args.gene
        if not gene:
            message = "--gene is required for the ppi module. Skipping PPI."
            warnings.append(message)
            print(f"[WARN] {message}")
        else:
    ...
    result = {
        ...
        "query": {
            ...
            "highlight_residues": args.highlight_residues,
            "highlight_secondary": args.highlight_secondary,
            "highlight_color": args.highlight_color,
            "highlight_labels": not args.no_highlight_labels,
        },
        "structure": {
            "source": source,
            "structure_id": prefix,
            ...
            "chain_used": chain_id,
            "n_residues": int(len(res_df)),
        },
        "summary": summary,
        "outputs": collect_generated_outputs(args.outdir, prefix),
        "warnings": warnings,
    }
